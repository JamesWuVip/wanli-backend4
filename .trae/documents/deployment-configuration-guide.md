...
```bash
# 数据库配置
DATABASE_URL=jdbc:postgresql://gondola.proxy.rlwy.net:55880/railway
DATABASE_USERNAME=postgres
DATABASE_PASSWORD=[Railway自动生成]
...
spring:
  ...
  datasource:
    url: ${DATABASE_URL:jdbc:postgresql://localhost:5432/wanli_backend_prod}
...
#### 1.3. 数据库不存在
**症状**: `database "xxx" does not exist`
**解决方案**:
```bash
# 连接PostgreSQL并创建数据库
psql -U postgres
CREATE DATABASE wanli_backend_dev;
CREATE DATABASE wanli_backend_staging;
```
...
- **部署平台**: Railway
- **数据库**: PostgreSQL 17.6
- **重要更新**: 修正所有环境使用PostgreSQL，统一环境变量命名，完善故障排除指南
